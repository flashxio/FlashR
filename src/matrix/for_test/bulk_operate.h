...
public:
	...
	virtual void runA(size_t num_eles, const void *in_arr,
			void *out_arr) const {
		size_t in_size = input_entry_size();
		size_t out_size = output_entry_size();
		const char *in_arr1 = (const char *) in_arr;
		char *out_arr1 = (char *) out_arr;
		for (size_t i = 0; i < num_eles; i++) {
			runE(in_arr1 + in_size * i, out_arr1 + out_size * i);
		}
	}
	virtual void runE(const void *in, void *out) const {
	}
...
public:
	...
	virtual void runEE(const void *left, const void *right, void *out) const {
	}
	...
	virtual void runAA(size_t num_eles, const void *left_arr,
		...
		size_t left_size = left_entry_size();
		size_t right_size = right_entry_size();
		size_t out_size = output_entry_size();
		const char *left_arr1 = (const char *) left_arr;
		const char *right_arr1 = (const char *) right_arr;
		char *output_arr1 = (char *) output_arr;
		for (size_t i = 0; i < num_eles; i++)
			runEE(left_arr1 + left_size * i, right_arr1 + right_size * i,
					output_arr1 + out_size * i);
	}
	/*
	...
	virtual void runAE(size_t num_eles, const void *left_arr,
		...
		char *output_arr1 = (char *) output_arr;
		for (size_t i = 0; i < num_eles; i++)
			runEE(left_arr1 + left_size * i, right, output_arr1 + out_size * i);
	}
	/*
	...
	virtual void runEA(size_t num_eles, const void *left,
		...
		char *output_arr1 = (char *) output_arr;
		for (size_t i = 0; i < num_eles; i++)
			runEE(left, right_arr1 + right_size * i, output_arr1 + out_size * i);
	}
	...
	virtual void runAgg(size_t num_eles, const void *left_arr,
			void *output) const {
		assert(get_left_type() == get_output_type());
		size_t left_size = left_entry_size();
		const char *left_arr1 = (const char *) left_arr;
		memcpy(output, left_arr, left_size);
		for (size_t i = 1; i < num_eles; i++)
			runEE(left_arr1 + left_size * i, output, output);
	}
