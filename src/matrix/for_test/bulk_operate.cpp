...
public:
	...
	virtual void runE(const void *in, void *out) const {
		const InType *in1 = (const InType *) in;
		OutType *out1 = (OutType *) out;
		*out1 = op(*in1);
	}
...
//		ResType tmp[tmp_len] __attribute__ ((aligned (16)));
//		left_arr = (const LeftType *) __builtin_assume_aligned(left_arr, tmp_len * sizeof(LeftType));
...
public:
	virtual void runEE(const void *left, const void *right, void *out) const {
		const LeftType *left1 = (const LeftType *) left;
		const RightType *right1 = (const RightType *) right;
		ResType *out1 = (ResType *) out;
		*out1 = op(*left1, *right1);
	}
